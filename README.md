...
|---------|------|-------------|
| 7.4.6 | 2026-09-23 | **fix: strip AppImage bundled libs from the Playwright browser environment.** The AppImage exports `LD_LIBRARY_PATH` pointing at its Ubuntu lib dir, which Chromium inherited; the bundled `libnssutil3` clashed with the host `libsoftokn3`, so NSS init failed and every HTTPS navigation died with SIGTRAP. Browser launches now scrub bundled entries (preserving real ones) via `env` on the shared launch kwargs, for every engine. |
